...
- [ADR-005: 割引種別を Enum（DiscountType）で型定義する](docs/adr/adr-005-discount-type-enum-definition.md)
...
- [ADR-009: 割引ポリシー注入時の防御コピー（List.copyOf）導入](docs/adr/adr-009-defensive-copy-for-injected-policies.md)
...
- テストスイート
	- anchor:代表ケース
		― OrderServiceGuardTest:Guard要件テスト
		- OrderServiceNormalTest:正常系テスト
		- OrderServiceFlowTest:port呼出順確認テスト
		― OrderServiceAbnormalTest:外部依存での異常時テスト
		- DiscountRules:割引計算確認テスト
		- DiscountLabels:割引ラベル確認テスト
		- TaxCalculation:税計算確認テスト
